...
def segment(sentences, video_duration, scenes=None):
    ...
    if scenes:
        scene_boundaries = {sc.start_time for sc in scenes if sc.start_time > 0}
        for i in range(len(sentences) - 1):
            for boundary in scene_boundaries:
                if sentences[i].end <= boundary <= sentences[i + 1].start:
                    boundary_pairs.add((sentences[i].index, sentences[i + 1].index))
                    break
                if sentences[i].start < boundary < sentences[i + 1].end:
                    boundary_pairs.add((sentences[i].index, sentences[i + 1].index))
                    break
    ...
    for i in range(n):
        ...
        for j in range(i, n):
            if j > i:
                pair = (sentences[j - 1].index, sentences[j].index)
    ...
    log.info(f"[SEGMENT] {len(windows)} windows ({min_dur}-{max_dur}s), "
             f"{len(boundary_pairs)} boundary pairs enforced")
    return windows
